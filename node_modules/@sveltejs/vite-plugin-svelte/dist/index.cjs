...
async function loadSvelteConfig(viteConfig, inlineOptions) {
  ...
  if (configFile) {
    ...
    if (!configFile.endsWith(".mjs")) {
      try {
        const _require = import_meta.url ? esmRequire != null ? esmRequire : esmRequire = (0, import_module.createRequire)(import_meta.url) : require;
...
function getSvelteDependencies(deps, pkgDir, path6 = []) {
  ...
  for (const { pkg, dir } of resolvedDeps) {
    result.push({ name: pkg.name, pkg, dir, path: path6 });
    if (pkg.dependencies) {
...
function resolveViaPackageJsonSvelte(importee, importer) {
  if (importer && isBareImport(importee)) {
    ...
    const importeePkg = JSON.parse(import_fs4.default.readFileSync(importeePkgFile, { encoding: "utf-8" }));
    ...
    load(id, ssr) {
      ...
      if (svelteRequest) {
        ...
        if (viteConfig.assetsInclude(filename)) {
          ...
          return import_fs5.default.readFileSync(filename, "utf-8");
        }
      }
    },
